...
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		var blog = $('#table_id').DataTable({
			...
			lengthChange: true,
			ajax: {
				url: base_url + "blog/get_list_blog/",
			},
			columns: [{
				'data': 'no'
			},
				...
				},
				{
					...
					'render': function (data, type, row) {
						return '<div class="btn-group" role="group" aria-label=""><a href="' + data +
							'" class="btn btn-success" title="Lihat Blog"><i class="icon icon-eye"></i></a><a href="<?php echo base_url() ?>blog/detail/' +
							data +
		...
		$(document).on('click', '#delete_blog', function () {
			...
			swal({
				...
				if (result.value) {
					window.location.replace('<?php echo base_url("blog/delete_post/")?>' + id);
				}
			});
		});
	});
